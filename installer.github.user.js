...
(function () {
  ...
  BLUL.createImportModuleFunc = function (context, keepContext = false) {
    /**
     * 如果需要上下文, Module 应当返回(export default)一个 Function/AsyncFunction, 其参数表示上下文, 且第一个参��是importModule
     * 在不需要上下文的情况下可以返回任意
     */
  ...
  BLUL.run = async (options) => {
    ...
    BLUL.INFO.__NEPTUNE_IS_MY_WAIFU__ = window.__NEPTUNE_IS_MY_WAIFU__; // 包含B站自己请求返回的一些数据，当然也���以自行请求获取
